...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedProjects) {
        ...
        if (currentProject) {
          // Ensure topics have indices for reliable selection
          if (currentProject.topics && Array.isArray(currentProject.topics)) {
            currentProject.topics = currentProject.topics.map((topic: any, idx: number) => ({
              ...topic,
              _index: idx // Add reliable index property
            }))
          }
  ...
  const selectAll = () => {
    if (project?.topics) {
      const allIndices = new Set<number>(project.topics.map((_: any, idx: number) => idx))
  ...
  const handleGenerateScripts = () => {
    ...
    const topicIndicesArray = Array.from(selectedTopics)
    console.log('✅ Topic indices being saved:', topicIndicesArray)

    // Save both topic indices and production mode
    localStorage.setItem('selectedTopicIndices', JSON.stringify(topicIndicesArray))
    localStorage.setItem('productionMode', productionMode)
    console.log('💾 Saved to localStorage - topic indices and production mode')

    const verification = localStorage.getItem('selectedTopicIndices')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                {topicsCount} topic{topicsCount !== 1 ? 's' : ''} • {scriptsCount} script{scriptsCount !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
        </div>
        ...
        {/* Topics List with Checkboxes - FIXED */}
        {topicsCount > 0 ? (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-sm text-gray-500">
                💡 Click on a topic card or checkbox to select it for script generation
              </p>
            </div>
          </div>
        ) : (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">
              Generate topics for this project to get started
            </p>
            <Link
